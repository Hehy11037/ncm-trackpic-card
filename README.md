...
一个把网易云音乐客户端的播放状态镜像到桌面悬浮卡片的工具：正面是封面卡片，翻面是逐字滚动的歌词，
托盘图标常驻，鼠标移上去才展开。

```
┌──────────────┐        正面：封面 / 标题 / 歌手 / 进度 / 播放控制 / 播放模式 / 音量
│   封面卡片    │        背面：逐字歌词（含翻译），按当前播放位置滚动
│   ▶  ─────   │        收起：只剩一条迷你播放条
└──────────────┘
```
...
1. 运行 `NCM Trackpic Card-<版本>-setup.exe`（按用户安装，不需要管理员）；
...
- 安装包未做代码签名，首次运行 Windows SmartScreen 会提示一次。
