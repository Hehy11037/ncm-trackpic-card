...
<p align="center">
  <img src="images/front1.png" width="300"> <img src="images/front2.png" width="300">
</p>
...
2. 装完会自动启动一次；**不会**设置开机自启（想让它开机就跑的话，自己在 Windows 设置 → 应用 → 启动 里加）；
...
| 操作 | 效果 |
| --- | --- |
| 鼠标移上去 | 展开卡片；移开自动收起（`L` 或点击锁键可锁定不收起） |
| 左上角圆圈箭头 | 翻面 / 翻回（`F`） |
| 左上角 🖼 | 自选封面：左键选图（再点=开关），右键清除 |
| 托盘图标 | 单击显示 / 隐藏；右键菜单选择卡片大小或退出程序 |
| 色带 | 可从五种颜色单击任选背景颜色 |
...
- 只在 Windows 上测过；调试通道、托盘、媒体键都是 Windows 行为；
- 客户端更新后调试通道的模块编号可能变化（宿主按形状发现模块，不写死编号）；
- 自定义封面暂只支持.jpg，.png文件，非正方形图片会居中截取；
- 网易云不支持滚动的歌词无法在歌词页显示。
